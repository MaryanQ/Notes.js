...
function start() {
  ...
  document.querySelector("#sort-form").addEventListener("change", sortByAge);
}
...
function sortByAge() {
  const selectValue = document.querySelector("#sortorder").value;

  let sortBy;

  if (selectValue === "ascending") {
    const sortByHighToLow = animals.sort((a, b) => a.age - b.age);
  } else if (selectValue === "descending") {
    const sortByLowToHigh = animals.sort((a, b) => b.age - a.age);
  }

  showAnimals(selectValue);
}
